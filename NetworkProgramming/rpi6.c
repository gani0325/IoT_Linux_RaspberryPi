...
/*
HTML 문서 안에 온도와 습도 등을 직접 표시할 수 있도록 삽입함 + lED 켜고 끌 수 있는 라디오 버튼 + 음악을 연주하고 모터를 제어
웹 브라우저에서 웹 서버에 데이터를 보낼 때 <FORM> 태그를 사용한다
<FORM> 태그는 POST 메시지로 요청을 보내지만, 앞 웹 서버는 GET 메소드만 처리하기 때문에
<FORM> 태그의 method 속성을 이용하여 GET 메소드로 보내도록 설정한다

웹 브라우저에서 웹 페이지를 불러들여서 옵션을 선택하고 Submit 버튼을 선택하면, 웹 서버로 
'http://서버주소:포트번호/index.html?led=Off' 요청의 GET 메소드로 전달된다
웹 서버에서는 GET 메소드로 전송된 메시지를 분석하고 라즈베리 파이의 GPIO 에 해당 명령을 내릴 수 있다

코드를 빌드 시 Pthread 라이브러리, wiringPi 라이브러리 모두 링크해야 한다
쓰니는 온습도 측정 센서가 없어서 작동하지 않는다 ㅠ
