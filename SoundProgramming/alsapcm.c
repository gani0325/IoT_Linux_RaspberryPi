...
/*
소리를 출력하기 위해서는 출력을 위한 버퍼의 메모리 공간을 할당한다
필요한 미모리 공간은 다음의 공식을 이용해서 계산할 수 있다
오디오의 크기 = 표본화율 X 양자화 눈큼의 크기 (비트 수) X 모드 (모노/스테레오) X 시간 (초)

pPCM 데이터를 저장하기 위해 필요한 버의 크기를 매크로로 정희하였다
양ㅇ자화 비트 16비트, 샘플링 주파수 44.1KHz, 채널은 모노 (1)을 사용했다
D
*/
D
